...
func (m *MockStorage) ListBuckets() ([]types.BucketInfo, error) {
	args := m.Called()
...
// 基本对象操作
func (m *MockStorage) PutObject(bucket string, object *types.S3ObjectData) error {
	args := m.Called(bucket, object)
	return args.Error(0)
}

func (m *MockStorage) GetObject(bucket, key string) (*types.S3ObjectData, error) {
...
func (m *MockStorage) ListObjects(bucket, prefix, marker, delimiter string, maxKeys int) ([]types.S3ObjectInfo, []string, error) {
...
// 分段上传
func (m *MockStorage) CreateMultipartUpload(bucket, key, contentType string, metadata map[string]string) (string, error) {
	args := m.Called(bucket, key, contentType, metadata)
	return args.String(0), args.Error(1)
}

func (m *MockStorage) UploadPart(bucket, key, uploadID string, partNumber int, data []byte) (string, error) {
	args := m.Called(bucket, key, uploadID, partNumber, data)
	return args.String(0), args.Error(1)
}

func (m *MockStorage) CompleteMultipartUpload(bucket, key, uploadID string, parts []types.MultipartPart) (string, error) {
