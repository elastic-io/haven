...
func (r *RegistryAPI) handlePushBlob(c *fiber.Ctx) error {
	...
	if len(c.Body()) > 0 {
		...
		if c.Request().Header.ContentLength() > r.streamThreshold {
			...
			// 写入数据到临时文件
			if _, err := tempFile.Write(c.Body()); err != nil {
			...
			// 读取整个文件
			blobData, err = io.ReadAll(tempFile)
			if err != nil {
				return c.Status(fiber.StatusInternalServerError).SendString("Failed to process data")
			}
		} else {
		...
		// 如果没有请求体，从临时存储获取之前通过PATCH上传的数据
		uploadData := getUploadData(repository, uuid)
		if uploadData == nil {
			return c.Status(fiber.StatusNotFound).SendString("No upload data found")
		}

		blobData, err = uploadData.GetData()
		if err != nil {
			return c.Status(fiber.StatusInternalServerError).SendString("Failed to retrieve upload data")
		}
	...
	if err := r.service.PutBlob(repository, digest, blobData); err != nil {
